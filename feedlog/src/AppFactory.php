...
use FeedLog\Feed\FeedRepository;
use FeedLog\Feed\RouteRegistrar;
...
class AppFactory
{
    ...
    public static function createMysql(string $host, string $name, string $user, string $password, int $port = 3306): RequestHandlerInterface
    {
        $dbConfig = new DatabaseConfig(
            url: null,
            environment: 'test',
    ...
    private static function build(DatabaseConfig $dbConfig): RequestHandlerInterface
    {
        ...
        $repo = new FeedRepository($executor);
